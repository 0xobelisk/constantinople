module constantinople::position_schema {
	...
	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address, x: u64, y: u64) {
		...
		if(table::contains<address, PositionData>(&_obelisk_schema.data, _obelisk_entity_key)) {
			*table::borrow_mut<address, PositionData>(&mut _obelisk_schema.data, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public(friend) fun set_x(_obelisk_world: &mut World, _obelisk_entity_key: address, x: u64) {
		let _obelisk_schema = world::get_mut_schema<SchemaMetadata>(_obelisk_world, id());
		assert!(table::contains<address, PositionData>(&_obelisk_schema.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow_mut<address, PositionData>(&mut _obelisk_schema.data, _obelisk_entity_key);
		_obelisk_data.x = x;
		events::emit_set(string(NAME), _obelisk_entity_key, *_obelisk_data)
	}
	...
	public fun get(_obelisk_world: &World ,_obelisk_entity_key: address): (u64,u64) {
  		...
  		assert!(table::contains<address, PositionData>(&_obelisk_schema.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, PositionData>(&_obelisk_schema.data, _obelisk_entity_key);
		(
			_obelisk_data.x,
			_obelisk_data.y
		)
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<SchemaMetadata>(_obelisk_world, id());
		table::contains<address, PositionData>(&_obelisk_schema.data, _obelisk_entity_key)
	}
