module constantinople::movable_schema {
	use std::option::some;
	...
	const SCHEMA_ID: vector<u8> = b"movable";
	...
	public fun register(_obelisk_world: &mut World, admin_cap: &AdminCap, ctx: &mut TxContext) {
		world::add_schema<Table<address,MovableData>>(_obelisk_world, SCHEMA_ID, table::new<address, MovableData>(ctx), admin_cap);
	}

	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address,  value: bool) {
		let _obelisk_schema = world::get_mut_schema<Table<address,MovableData>>(_obelisk_world, SCHEMA_ID);
		let _obelisk_data = new( value);
		if(table::contains<address, MovableData>(_obelisk_schema, _obelisk_entity_key)) {
			*table::borrow_mut<address, MovableData>(_obelisk_schema, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public fun get(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<Table<address,MovableData>>(_obelisk_world, SCHEMA_ID);
		assert!(table::contains<address, MovableData>(_obelisk_schema, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, MovableData>(_obelisk_schema, _obelisk_entity_key);
		(
			_obelisk_data.value
		)
	}
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<Table<address,MovableData>>(_obelisk_world, SCHEMA_ID);
		table::contains<address, MovableData>(_obelisk_schema, _obelisk_entity_key)
	}
}
