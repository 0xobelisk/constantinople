module constantinople::encounter_schema {
	...
	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address, exists: bool, monster: address, catch_attempts: u64) {
		...
		if(table::contains<address, EncounterData>(&_obelisk_schema.data, _obelisk_entity_key)) {
			*table::borrow_mut<address, EncounterData>(&mut _obelisk_schema.data, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public(friend) fun set_exists(_obelisk_world: &mut World, _obelisk_entity_key: address, exists: bool) {
		let _obelisk_schema = world::get_mut_schema<SchemaMetadata>(_obelisk_world, id());
		assert!(table::contains<address, EncounterData>(&_obelisk_schema.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow_mut<address, EncounterData>(&mut _obelisk_schema.data, _obelisk_entity_key);
	...
	public fun get(_obelisk_world: &World ,_obelisk_entity_key: address): (bool,address,u64) {
  		...
  		assert!(table::contains<address, EncounterData>(&_obelisk_schema.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, EncounterData>(&_obelisk_schema.data, _obelisk_entity_key);
		(
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<SchemaMetadata>(_obelisk_world, id());
		table::contains<address, EncounterData>(&_obelisk_schema.data, _obelisk_entity_key)
	}
