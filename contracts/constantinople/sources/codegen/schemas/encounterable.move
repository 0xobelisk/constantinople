module constantinople::encounterable_schema {
	...
	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address, value: bool) {
		...
		if(table::contains<address, EncounterableData>(&_obelisk_schema.data, _obelisk_entity_key)) {
			*table::borrow_mut<address, EncounterableData>(&mut _obelisk_schema.data, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public fun get(_obelisk_world: &World ,_obelisk_entity_key: address): bool {
  		...
  		assert!(table::contains<address, EncounterableData>(&_obelisk_schema.data, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, EncounterableData>(&_obelisk_schema.data, _obelisk_entity_key);
		(
			_obelisk_data.value
		)
	}
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<SchemaMetadata>(_obelisk_world, id());
		table::contains<address, EncounterableData>(&_obelisk_schema.data, _obelisk_entity_key)
	}
