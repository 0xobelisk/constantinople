module constantinople::map_comp {
	...
	public(friend) fun set(_obelisk_world: &mut World,  width: u64, height: u64, terrain: vector<vector<u8>>) {
		...
		if(table::contains<address, MapData>(&_obelisk_component.data, id())) {
			*table::borrow_mut<address, MapData>(&mut _obelisk_component.data, id()) = _obelisk_data;
		} else {
	...
	public(friend) fun set_width(_obelisk_world: &mut World,  width: u64) {
		let _obelisk_component = world::get_mut_comp<CompMetadata>(_obelisk_world, id());
		assert!(table::contains<address, MapData>(&_obelisk_component.data, id()), EEntityDoesNotExist);
		let _obelisk_data = table::borrow_mut<address, MapData>(&mut _obelisk_component.data, id());
